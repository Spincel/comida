        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div class="lg:col-span-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <button v-if="isSessionOpen" @click="closeCurrentSession" class="bg-rose-600 hover:bg-rose-700 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-rose-500/20 flex items-center gap-2 animate-blink-danger transition-all hover:scale-105 active:scale-95">
                        <PowerIcon class="h-4 w-4" /> Finalizar {{ mealType }}
                    </button>
                    <!-- Badge indicador si la sesión ya está cerrada -->
                    <div v-else class="flex items-center gap-2 px-5 py-3 rounded-2xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 text-emerald-600 dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest shadow-sm">
                        <CheckCircleIcon class="h-4 w-4 text-emerald-500" />
                        <span>Turno Finalizado</span>
                    </div>

                    <!-- Enlace para volver al Dashboard -->
                    <Link :href="route('dashboard')" class="bg-slate-100 hover:bg-slate-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-slate-700 dark:text-gray-200 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-sm flex items-center gap-2 transition-all hover:scale-105 active:scale-95">
                    ...
                    <div class="flex bg-slate-100 dark:bg-gray-800 p-1.5 rounded-2xl border dark:border-gray-700">
                        <button @click="viewMode = 'compact'" :class="viewMode === 'compact' ? 'bg-white dark:bg-gray-700 text-indigo-600 shadow-sm' : 'text-slate-400'" class="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Compacto</button>
                        <button @click="viewMode = 'detailed'" :class="viewMode === 'detailed' ? 'bg-white dark:bg-gray-700 text-indigo-600 shadow-sm' : 'text-slate-400'" class="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Detallado</button>
                    </div>
                    <button @click="openExportModal()" class="bg-slate-900 dark:bg-indigo-600 hover:bg-slate-800 dark:hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg flex items-center gap-2 transition-all hover:scale-105 active:scale-95"><PrinterIcon class="h-4 w-4" /> Exportar</button>
                    <button @click="sendToWhatsApp()" class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg flex items-center gap-2 transition-all hover:scale-105 active:scale-95"><ChatBubbleLeftRightIcon class="h-4 w-4" /> WhatsApp</button>
                </div>
            </div>
